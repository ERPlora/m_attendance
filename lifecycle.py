"""
Attendance module lifecycle hooks.
...
async def on_install(session: AsyncSession, hub_id: UUID) -> None:
    """Called after module installation + migration. Seed default settings."""
    ...
    await session.flush()
    logger.info("Attendance module installed for hub %s -- default settings created", hub_id)


async def on_activate(session: AsyncSession, hub_id: UUID) -> None:
    """Called when module is activated."""
    logger.info("Attendance module activated for hub %s", hub_id)


async def on_deactivate(session: AsyncSession, hub_id: UUID) -> None:
    """Called when module is deactivated."""
    logger.info("Attendance module deactivated for hub %s", hub_id)


async def on_uninstall(session: AsyncSession, hub_id: UUID) -> None:
    """Called before module uninstall."""
    logger.info("Attendance module uninstalled for hub %s", hub_id)
...
async def on_upgrade(session: AsyncSession, hub_id: UUID, from_version: str, to_version: str) -> None:
    ...
    logger.info(
        "Attendance module upgraded from %s to %s for hub %s",
